...
import { fetchInstancesByYearSemester, fetchAllInstances, fetchCourses, deleteInstance as apiDeleteInstance } from '@/lib/api';
...
export default function InstancesPage() {
    ...
    const loadInstances = useCallback(
        async (filtersToApply?: InstanceFilterValues) => {
            setIsLoading(true);
            setError(null);
            try {
                let data: CourseInstanceResponse[];

                if (!showAll && filtersToApply && filtersToApply.year && filtersToApply.semester) {
                    data = await fetchInstancesByYearSemester(filtersToApply.year, filtersToApply.semester);
                } else {
                    data = await fetchAllInstances(); // default or cleared filters
                }
                ...
                toast({
                    title: "Error",
                    description: `Failed to load instances: ${errorMessage}`,
                    variant: "destructive",
                });
            } finally {
                setIsLoading(false);
    ...
    return (
        <AppLayout>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        ...
                        <p className="text-muted-foreground">
                            Manage and schedule course delivery instances.
                        </p>
                    </div>
                    <Button onClick={handleCreateNew} className="shadow-md hover:shadow-lg transition-shadow" disabled={courses.length === 0 && !isLoading}>
            ...
            <InstanceForm
                ...
                onSuccess={() => {
                    setShowAll(false);        // default behavior
                    handleFormSuccess();
                }}
                courses={courses}
            ...
            <InstanceDetailsModal
                ...
                onClose={() => { setIsDetailsModalOpen(false); setViewingInstance(null); }}
            />

            <DeleteInstanceDialog
                instance={deletingInstance}
                isOpen={isDeleteAlertOpen}
                onClose={() => { setIsDeleteAlertOpen(false); setDeletingInstance(null); }}
